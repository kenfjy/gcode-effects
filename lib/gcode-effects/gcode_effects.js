...
var Dimention = function Dimention(max_x, min_x, max_y, min_y, max_z, min_z) {
...
function sDimention(geometry, geom_z, height) {
	...
	if (geometry.length == 0) {
		...
		var d = new Dimention(max_x, min_x, max_y, min_y, geom_z + height, geom_z);
		return d;
	}
}

// getting dimention object from gcode
function gDimention(gcode) {
	...
	if (!_trig_x || !_trig_y || !_trig_z) {
		...
		var d = new Dimention(max_x, min_x, max_y, min_y, max_z, min_z);
		return d;		
	}
}

function gIntersect(t_gcode, r_dim, r_geometry) {
