...
function generateInsert(height = 4.0, resolution = 16, _x = null, _y = null, _z = null, type = "TAG", param = 5.0) {
	if (!_x || !_y || !_z) {
		var gcode_a = gcodeToArray(localStorage.getItem('last-imported'));
		var bbox = gcodeBoundingBox(gcode_a);
		_x = bbox.x;
		_y = bbox.y;
		_z = 5.0;
	}
	...
	if (type == "TAG") {
		...
		// shape = createTag(param, base_point, height, resolution);
...
function shapeToThreeObject(object) {
	var object = new THREE.Object3D();
	var geometry = new THREE.Geoemetry();
   var color = new THREE.Color(internalColor);
	shape = object.shape;
	dim = object.dimension;
	for(i=0;i<shape.length;i++) {
		for(j=dim.z;j<(dim.z+dim.h);j+=(dim.h/20)) {
			geometry.vertices.push(new THREE.Vector3(shape[i][0].x, shape[i][0].y, j));
			geometry.vertices.push(new THREE.Vector3(shape[i][1].x, shape[i][1].y, j));
